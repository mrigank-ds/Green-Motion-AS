(function (global, factory) {
	...
	(global = global || self, global.RtfConverter = factory());
	...
	var markdownItPluginUnderline = function( md) {
	...
	/**
	 * A utility class that has methods to convert Yext's flavor of Markdown
	 * into various other formats. Currently, the only format supported is
	 * HTML. The markdown-it library and associated plugins are used to perform
	 * all conversions.
	 */

	var YextRtfConverter = /*#__PURE__*/function () {
	  function YextRtfConverter() {
	    this._md = new MarkdownIt();
	    this.addPlugin(markdownItPluginUnderline);
	    this.addPlugin(markdownItSub);
	    this.addPlugin(markdownItSup);
	  }
	  /**
	   * Registers a new plugin for use with the internal instance of markdown-it.
	   * Once registered, the plugin will be applied during any Markdown conversions.
	   * 
	   * @param {Function} plugin A markdown-it plugin.
	   * @param {Array<?>} params Any additional parameters needed to initialize
	   *                          the plugin.
	   */


	  var _proto = YextRtfConverter.prototype;
	  ...
	  _proto.addPlugin = function addPlugin(plugin) {
	    ...
	    params.unshift(plugin);

	    this._md.use.apply(this._md, params);
	  }
	  /**
	   * Disables the specified plugin in the internal instance of markdown-it. Once
	   * disabled, the plugin will no longer be applied during any Markdown conversions.
	   * 
	   * @param {string} name The name of the plugin.
	   */
	  ;

	  _proto.disablePlugin = function disablePlugin(name) {
	    this._md.disable(name, true);
	  };

	  _proto.toHTML = function toHTML(markdown) {
	    var html = this._md.render(markdown);

	    return html;
	  };

	  return YextRtfConverter;
	}();

	var RtfConverter= new YextRtfConverter();
	RtfConverter["class"] = YextRtfConverter;
